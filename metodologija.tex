...
\begin{equation}
    ...
    \begin{cases}
        <1 &\text{$A$ in $B$ se prekrivata,}\\
        =1 &\text{$A$ in $B$ sta tangentni,}\\
        >1 &\text{$A$ in $B$ se ne dotikata.}
    \end{cases}
    \label{eq:kriterij}
\end{equation}
...
Spremljamo energijo sistema, povprečno število sosedov, delež sprejetih oziroma 
zavrnjenih rotacij ter orientacijsko korelacijsko funkcijo.
\subsection{Energija}
Energija celotnega sistema je vsota energij posameznih elips $E = \sum u_i$. Elipsi 
pripišemo energijo, ki je je večja od nič, če se elipsi prekrivata in enaka nič, če se 
elipsi ne prekrivata ali pa se le dotikata. To dosežemo na sledeči način
\begin{equation}
    E = \frac{1}{2} \sum\limits_{i} u_i = - \frac{1}{2} \sum\limits_{i,j} \log
    \begin{cases}
        \mu_{i,j}^2    &\mu_{ij} < 1,\\ 
        1               &\mu_{ij} \geq 1.
    \end{cases}
...
\subsection{Orientacijska korelacijska funkcija}

\begin{align}
    g_2 (\vec{r}, t) &= < \psi_{i,2} \cdot \psi_{j,2}^* >, \\
    \psi_{i,2} &= \frac{1}{Z_j} \sum\limits_{k=1}^{Z_j} \mathrm{e}^{\mathrm{i}\, 2 
                    \vartheta_{jk}},
\end{align}
kjer je $Z_j$ število sosedov elipse, $\vartheta_{jk}$ pa kot med glavnima polosema elips
$j$ in $k$.